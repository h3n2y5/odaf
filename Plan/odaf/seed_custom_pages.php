...
// -----------------------------------------------------------------------------
// 1. Point of Sale (POS) Custom Page
// -----------------------------------------------------------------------------
$posBlade = <<<BLADE
...
// -----------------------------------------------------------------------------
$promoBlade = <<<BLADE
<div class="h-full bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-sm">
...
// INJECTION LOGIC
// -----------------------------------------------------------------------------
function insertCustomPage(\$appId, \$pageCode, \$pageName, \$blade, \$php) {
    ...
    \$pageId = strtoupper(Str::uuid()->toString());
    
    DB::insert("
        INSERT INTO ODAF.UI_PAGE (OBJECT_ID, APPLICATION_ID, OBJECT_CODE, OBJECT_NAME, PAGE_TYPE, LAYOUT_TYPE, VERSION_NO, STATUS, CUSTOM_VIEW_BLADE, CUSTOM_LOGIC_PHP)
        VALUES (HEXTORAW(?), HEXTORAW(?), ?, ?, 'CUSTOM', 'BLANK', 1, 'PUBLISHED', ?, ?)
    ", [\$pageId, \$appId, \$pageCode, \$pageName, \$blade, \$php]);
    ...
    if (!\$mod) {
        \$modId = strtoupper(Str::uuid()->toString());
    ...
    echo "Inserted Custom Page: \$pageName (\$pageCode)\\n";
}
...
try {
    ...
    insertCustomPage(\$appIdHex, 'CUST_POS', 'Point of Sale (Kasir)', \$posBlade, \$posPhp);
    insertCustomPage(\$appIdHex, 'CUST_PROMO', 'Promo Simulator', \$promoBlade, \$promoPhp);
